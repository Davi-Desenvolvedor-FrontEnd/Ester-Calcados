    ...
    <div className="h-full w-full flex items-start justify-center bg-linear-to-br  py-8 ">
      <div className="w-full max-w-md bg-white rounded-md shadow-2xl p-8 transition-all duration-300 hover:shadow-[0_20px_70px_-15px_rgba(0,0,0,0.4)] ">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            {mode == "sign" && (
              <a
                href="#"
                className="text-sm font-medium text-pink-600 hover:text-pink-500 hover:underline transition"
              >
                Esqueceu a senha?
              </a>
            )}
        ...
        <p className="mt-6 text-center text-sm text-gray-600">
          {mode == "sign" ? "Não tem uma conta?" : "Tem uma conta?"}{" "}
          <a
            href="#"
            className="font-semibold text-pink-600 hover:text-pink-500 hover:underline transition"
          >
            <button
              type="button"
              className="border-0 cursor-pointer"
              onClick={() => {
                setMode(mode == "sign" ? "register" : "sign");
              }}
            >
              {mode == "sign" ? " Cadastre-se" : "Faça login"}
            </button>
          </a>
        </p>
      </div>
